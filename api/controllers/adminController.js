			...
			if(!pass){/////////////////////////////////////two
				...
				User.findOne({username: newUser.username}, function(err, user){
					...
					if(user){
						...
						//return res.render('azbycx/register', {admin: newUser});
					}
					  	...
					  	bcrypt.hash(newUser.password, salt, function(err, hash){
							...
							newUser.save(function(err, newUsr){
								if(err){
									...
									return res.redirect('/medusa123/login');
									//next();
								}
							});
						});
...
exports.update_admin_profile = function(req, res, next){
	...
	return res.redirect('/medusa123/profile');
}
...
exports.approve_admin_downline = function(req, res, next){
	...
	User.update({"incomingReservations.coupleId": coupleId}, {$set: {"incomingReservations.$.confirmation_flag" :  "Approved", "incomingReservations.$.imageName" :  "Approved", "incomingReservations.$.pop" :  "Approved"}}, function(err, updatedUpline){///**one******
			...
			if(updatedUpline){//**************************************two***********************************************************
				...
				// //Delete the image. Not necessary because i'm using cloudinary
			...
			if(updatedUpline){//**************************************two***********************************************************
				...
				User.update({_id: uplineId}, {$inc: {amountReceived: approvalAmount}}, function(err, reducedAmountRemaining){
					...
					if(reducedAmountRemaining){//******************************three*******************************************************
						...
						User.update({"outgoingReservations._id": coupleId}, {$set: {reactivated: 'none',
																					"outgoingReservations.$.confirmation_flag" :  "Approved", 
							...
							if(updatedDownline){//************************************five*************************************************************************
								...
								User.update({username: duname}, {$inc: {amountRemaining: approvalAmount*1.5, amountReserved: approvalAmount}}, function(err, amountUpdate){/////six********************
									...
									if(!amountUpdate){
										console.log({msg: 'downline amountRemaining increment failed!'});
										//return res.send({msg: 'Payment approval failed!'});
									}else{
										console.log({msg: amountUpdate});
										//return res.send({msg: 'Payment approve success'});
									}
...
exports.decline_admin_downline = function(req, res, next){
	...
	User.update({"incomingReservations.coupleId": coupleId},
		{$set: {"incomingReservations.$.confirmation_flag" :  "Declined", "incomingReservations.$.pop" :  "Declined"},
		...
		function(err, updatedUpline){
			...
			User.update({"incomingReservations.coupleId": coupleId},
				{$set: {"incomingReservations.$.confirmation_flag" :  "Declined", "incomingReservations.$.imageName" :  "Declined",
						"incomingReservations.$.pop" :  "Declined", "incomingReservations.$.resolved" :  "yes"}},
				function(err, updatedUpline){
					if(err){
						console.log(err);
						return res.send({msg: 'Ooops. Something went wrong...'});
					...
					User.update({"outgoingReservations._id": coupleId}, {$set: {reactivated: 'none', suspended: true, suspensionTime: suspensionTime, "outgoingReservations.$.confirmation_flag" :  "Declined",
...
exports.get_all_registered_users = function(req, res, next){
	User.find({admin: false}, function(err, users){
		if(err){
			console.log(err);
			return res.send({msg: 'Ooops. Something went wrong...'});	//else return user object for manipulation
		}
...
exports.purge = function(req, res, next){
	...
	console.log({purge1: 'one'});

	User.find({admin: false}, function(err, users){////////////////////0

		console.log({purge2: 'two'});
		var date = new Date();
		var now = date.getTime();
		var purge_count = 0;

		users.forEach(function(user){////////////////////////////////////////////////////////////1
			...
			if(outgoings != ''){////////////////////////////2
				var out, coupleId, upline, coupleIndex;
					...
					//GET LAST OUTGOING TRANSACTION OF THE USER.
				...
				//make sure you put succeeding codes inside any loop whatsoever to prevent it from running again after completion of the needing runs! VIP!!!!
				//or you include an 'if-statement" inside it to stop the loop'

				//IF USER HASN'T BEEN PURGED AND TIME FOR PAYMENT HAS ELAPSED, PURGE!
				if(user.purged == false && out.deadline < now && out.confirmation_flag == 'Pending'){///////////////////////////////3
					...
					User.findOne({username: upline}, function(err, up){/////////////////////4
						if(err){
							console.log(err);
							return res.send({msg: 'Ooops. Something went wrong. One'});
						}
						...
						User.update({_id: downlineId}, {$set: {purged: true}, 
							...
							function(err, success){/////////////////////5
								if(err){
									console.log(err);
									return res.send({msg: 'Ooops. Something went wrong. Two'});
								}

							//console.log({success1: success})
							if(!success){
								console.log({msg: 'Ooops! Cound not purge defaulter'});
							}

							purge_count = purge_count + 1;

							console.log({Purged_count_for_now: purge_count});
							...
							User.update({username: upline},
								...
								function(err, success){/////////////////////6
									if(err){
										console.log(err);
										return res.send({msg: 'Ooops. Something went wrong. Three'});
									}

									/*console.log({success2: success})

									if(success){
										status = 'Purge success!'; ///////////////finally, send the status to the client!
									}else{
										status = 'No defaulter to purge!'; ///////////////finally, send the status to the client!
									}*/
			...
			console.log({count: 'happy'});
			return res.send({msg: purge_count}); //RETURN PURGE COUNT!
	});////////////////////////0
}
			...
			User.update({"incomingReservations.coupleId": coupleId},//imageName
				...
				suspended: true, suspensionTime: suspensionTime},
				 ...
				 function(err, updatedUpline){///**one******
					...
					if(updatedUpline){//**************************************two***********************************************************
						//Delete the image.
						/*fs.unlink("../api/public/uploads/"+image, function(err){
							if(err){
								console.log(err);
								return res.send({msg: 'Ooops. Something went wrong...'});
							}
...
exports.reactivate_account = function(req, res, next){
	const username = req.body.username;
	User.findOneAndUpdate({username: username}, {$set: {suspended: false, suspensionTime: 0, reactivated: 'yes'}}, function(err, updated){
		if(err){
			console.log(err);
			return res.send({msg: 'Ooops. Something went wrong...'});
		}
